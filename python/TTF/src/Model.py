...
def AlexNet(input_shape, num_classes, learning_rate, graph):
    """
        ...
        input_shape: The shape of input (`list` like)  输入的形状（类似“ list”）
        num_classes: The number of output classes (`int`) 输出类的数量（`int`）
        learning_rate: learning rate for optimizer (`float`) 优化程序的学习率（“ float”）
        graph: The tf computation graph (`tf.Graph`) tf计算图（`tf.Graph`）
    """
